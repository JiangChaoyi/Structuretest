...
$.extend($, {
    Dialog: function(setting) {
            ...
            $('<div id="dialog-overlay" class="dialog-overlay"/>\
                <div class="dialog-main" id="dialog-main" >\
                    ...
                    <div class="dialog-middle">\
                        <div class="dialog-border-left dialog-png-fiexed">&nbsp;</div>\
                        ...
                        <div class="dialog-border-right dialog-png-fiexed">&nbsp;</div>\
                    </div>\
		...
		if(ps.buttonshow){
		  ...
		  }).next().val(ps.buttonText.cancel).one('click', function() { 
			   args.hide(); 
...
/*$.Dialog({ title: '消息提示', content: '<div class="errortip">'+objarray['message']+'</div>', fixed: true,location:false, buttonshow: true,okEvent: function(data, args) {
	  if(objarray['url'] !==''){
		  window.location=objarray['url'];
	  }else{
		  if(objarray['bak'] =='location'){
			  location.reload();
		  }
		  if(objarray['bak'] =='history'){
			  history.go(-1);
		  }
	  }
 }});*/
